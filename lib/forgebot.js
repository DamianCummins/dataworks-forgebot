...
ForgeBot.prototype._welcomeMessage = function(){
	this.postMessageToChannel(this.channels[0].name, 'Hi Team, ForgeBot here!' +
        '\n Find out what I can do by posting `' + this.name + ' commands`.',
        {as_user: true});
};
...
ForgeBot.prototype._listActivities = function(originalMessage, inFull){
	...
	dwInstance.listActivities(function(err, list){
		...
		if(!inFull){
			...
			self.postMessageToChannel(channel.name, "", {as_user:true, attachments: self._getActivitiesAttachment(response)});
		} else {
...
ForgeBot.prototype._runActivity = function(originalMessage, byName){
	...
	if(activityId === null && activityName === null){
		...
		dwInstance.runActivity(activityId, function(err, common){
			...
			var messageAttachment = self._getStatusAttachment(common);
			var channel = self._getChannelById(originalMessage.channel);
			self.postMessageToChannel(channel.name, "", {as_user:true, attachments: messageAttachment});
		});
...
ForgeBot.prototype._getStatusAttachment = function(common){
	var self = this;
	var statusColours = {
		"NONE": "#0000cc",
		"SUBMITTED": "#36a64f",
		"RUNNING": "#36a64f",
		"RUNNING_WITH_WARNINGS": "#eebb00",
		"RUNNING_WITH_ERRORS": "#cc0000",
		"FINISHED": "#36a64f",
		"FINISHED_WITH_WARNINGS": "#eebb00",
		"FINISHED_WITH_ERRORS": "#cc0000",
		"ABORTED": "#cc0000"
	}
	return [
	        {
	            "fallback": "Activity: " + common.name + "\nRun: " + common.id + "\nStatus: " + common.status,
	            "color": statusColours[common.status],
	            "title": "Activity Run Status",
	            "fields": [
	            	{
	            		"title": "Activity Name",
	            		"value": common.name,
...
ForgeBot.prototype._getActivitiesAttachment = function(activities){
	var self = this;
	var response = [];
	activities.forEach(function(activity){
		var attachment = {
	            "fallback": JSON.stringify(activity, null, 2),
	            "color": "#0000cc",
	            ...
	            "fields": [
	            	...
	            	{
	            		"title": "Created At",
	            		"value": activity.createdTimeStamp,
	            		"short": false
	            	}
	            ]
	        }
