	...
	if(this._isChatMessage(message) &&
		...
		this._isMentioningForgeBot(message)) {
			if(this._isUsage(message)){
				...
				if(this._isRunActivityByName(message)){
					this._runActivity(message, true);
				} else {
					this._runActivity(message, false);	
				}
...
ForgeBot.prototype._printUsage = function(originalMessage){
	...
	"Forgebot run activity named <activityName>\n" +
...
ForgeBot.prototype._isRunActivityByName = function(message){
	return message.text.toLowerCase().indexOf('run activity named') > -1;
};
...
ForgeBot.prototype._runActivity = function(originalMessage, byName){
	...
	if(!byName){
		...
		var namedIndex = tokens.indexOf("named");
...
ForgeBot.prototype._getActivityRunStatus = function(originalMessage){
	...
	var activityId = null;
	if (tokens[runIndex+4] == "activity"){
		activityId = tokens[runIndex + 5];
	}
